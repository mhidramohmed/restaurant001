...
const EditMenuItemModal = ({ categoryId, itemId, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Ensure category_id is always passed, prioritizing the prop or fallback to existing category_id
        const categoryToUse = categoryId || formData.category_id
        if (!categoryToUse) {
            toast.error('Category ID is required')
            setIsSaving(false)
            return
        }
        data.append('category_id', categoryToUse)
