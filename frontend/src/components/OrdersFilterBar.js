...
// Enhanced Filtering for Orders Page
const OrdersFilterBar = ({ filters, setFilters, sortNewestFirst, setSortNewestFirst }) => {
  ...
  const dateRanges = [
    ...
    { label: 'Last 3 days', value: 'last3d' },
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          ...
          >
            ...
            <span className="flex items-center">
              {filters.paymentStatuses.length > 0 && (
                <span className="bg-green-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center mr-1">
      ...
      {(filters.statuses.length > 0 || filters.paymentStatuses.length > 0 || filters.dateRange || filters.searchTerm) && (
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {filters.paymentStatuses.map(status => (
            <span key={status} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-500 text-white">
              {status.charAt(0).toUpperCase() + status.slice(1)}
              <X 
                size={14} 
                className="ml-1 cursor-pointer" 
                onClick={() => setFilters(prev => ({
                  ...prev,
                  paymentStatuses: prev.paymentStatuses.filter(s => s !== status)
                }))}
              />
            </span>
          ))}
